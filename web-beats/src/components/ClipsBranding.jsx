...
export function ZeusClipsWordmark({ size = 'clamp(13px, 4.8vw, 19px)', to, word = 'CLIPS' }) {
  const { user } = useAuth();
...
export function ClipsAiBadge() {
  return (
    <div className="clips-ai-badge" style={{
...
export function ClipGenrePill({ genre, style }) {
  ...
  return (
    <span style={{
      ...
      maxWidth: '100%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', verticalAlign: 'top',
...
export function BackToBeatsLink({ style }) {
  ...
  return (
    ...
    >
      {/* Narrow / zoomed screens: just the arrow (index.css). */}
      <span className="beats-back-long">← Zeus Beats</span>
      <span className="beats-back-short" aria-hidden="true">←</span>
    </Link>
  );
}
