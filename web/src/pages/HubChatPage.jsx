...
function Answer({ request }) {
  ...
  return <article className="hub-turn">
    ...
    {result && <div className="hub-answer"><span className="hub-kicker">{request.feature === 'council' ? '🧠 ZEUS VERDICT' : '⚡ ZEUS'}</span>
      {result.simulated && <p className="hub-beta-caption">Beta · simulated response</p>}
...
export default function HubChatPage({ feature = 'ask' }) {
  ...
  return <div className="hub-page"><DashboardHeader /><main className="hub-chat-layout">
    ...
    <section className="hub-chat-panel"><header className="hub-chat-heading"><p className="hub-kicker">{council ? 'PERSPECTIVE. CONSENSUS. CLARITY.' : 'YOUR NEXT IDEA STARTS HERE.'}</p><h1>{council ? '🧠 Zeus Council · Beta' : '⚡ Ask Zeus · Beta'}</h1>{council && <p className="hub-council-formula">3 AIs + 1 Zeus Verdict</p>}<p>{council ? 'Up to three independent perspectives, brought together in one clear answer. If a member is unavailable, Zeus can work with two responses.' : 'Ask naturally. Zeus selects the AI for your task.'}</p>
