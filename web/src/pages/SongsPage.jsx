...
const GENRE_CATEGORIES = [
  ...
  { id: 'soul',       label: '🎵 Soul & Blues',        color: '#fb923c',
    genres: ['soul','bluessoul','southemsoul','soulrnb','orchestralsoul','classicfunk','gospel','hymns','trapsoul','vocaljazz','swing','rnb','blues','deepsoulblues','chicagoblues','deltablues','swampblues'] },
  ...
  { id: 'world',      label: '🌍 World & Urban',       color: '#fbbf24',
    genres: ['afrobeats','reggae','rootsreggae','reggaeton','ragga','corridos','salsa','bhangra','loversrock','rastadub','amapiano','latintrap','brassband'] },
  ...
  { id: 'chill',      label: '🧘 Chill & Wellness',    color: '#e2e8f0',
    genres: ['lofi','meditation','ambient','healingfrequency','naturesounds','whalesong','cracklingfire','thunderstorm','oceanwaves','forest','nightsounds','classical','acoustic','jazz','irishfolk','irishjig','christmas','dixieland','harlemstride'] },
...
@keyframes glowPulse {
  ...
  50%      { box-shadow: 0 0 40px rgba(255,0,153,0.4); }
}
.cover-video { transition: filter 0.2s; }
.cover-video:hover { filter: brightness(1.12); }
.cover-ken-burns { animation: kenBurns 30s ease-in-out infinite; transform-origin: center; will-change: transform; }
.cover-ken-burns:hover { filter: brightness(1.08); }
.cover-glow-pulse { animation: glowPulse 4s ease-in-out infinite; }
      ...
      <div style={{ position: 'relative' }}>
        ...
        {favToast && (
          <div style={{
            ...
            {favToast === 'added' ? 'Saved to favourites ✅' : 'Removed from favourites'}
          </div>
        )}
      </div>
          ...
          <>
            ...
            {lockedMsg && (
              <div style={{
                ...
                background: 'rgba(18,18,30,0.96)', border: '1px solid rgba(167,139,250,0.25)',
                fontSize: 11, color: '#c4b5fd', textAlign: 'center', lineHeight: 1.5,
              }}>
                {lockedMsg === 'upgrade-yt' && <>Upgrade your plan to upload to YouTube. <Link to="/billing" style={{ color: '#a78bfa', fontWeight: 600 }}>Upgrade →</Link></>}
                {lockedMsg === 'connect-yt' && <>Connect YouTube in Settings to upload.</>}
                {lockedMsg === 'upgrade-avatar' && <>Upgrade to Music Pro for avatar videos. <Link to="/billing" style={{ color: '#a78bfa', fontWeight: 600 }}>Upgrade →</Link></>}
                {lockedMsg === 'no-avatar-credits' && <>No avatar credits remaining. <Link to="/billing" style={{ color: '#a78bfa', fontWeight: 600 }}>Top up →</Link></>}
              </div>
            )}
            {addToast && (
              <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
              <div style={{ position: 'relative' }} ref={addMenuRef}>
                <button
                  ...
                  style={{
                    background: 'none', border: '1px solid rgba(167,139,250,0.35)', borderRadius: 5,
                    color: '#a78bfa', fontSize: 11, cursor: 'pointer', padding: '3px 10px',
                    transition: 'all 0.15s',
                  }}
                >
                  ➕ Playlist
                ...
                {addMenuOpen && (
                  <div style={{
                    ...
                    background: '#0e0e1a', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 8,
...
export default function SongsPage() {
  ...
  const handleYouTubeUpload = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (!r.ok) {
        if (r.status === 401) fetchCredits(); // token expired — re-fetch resets youtube_connected
    ...
    <>
            ...
            {showAdvanced && (
              <div className="adv-grid" style={{
                ...
                {!['meditation','healingfrequency'].some(g => selGenres.has(g)) && <div>
                  ...
                  >
                    ...
                    {[
                      ...
                      'West Coast G-Funk', 'British Street Soul',
                    ].map((a) => (
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <p style={{ fontSize: 11, fontWeight: 600, color: '#555', letterSpacing: '0.6px', textTransform: 'uppercase', marginBottom: 6 }}>Avoid</p>
                  <input
                    type="text"
                    maxLength={500}
                    value={negativeTags}
                    onChange={(e) => setNegativeTags(e.target.value)}
                    placeholder="e.g. no piano, no trumpet (max 500 chars)"
                ...
                <div style={{ gridColumn: '1 / -1', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 16 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: genreBlend ? 14 : 0 }}>
                    ...
                    {genreBlend && genreB && selGenres.size > 0 && (
                      <span style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 20, background: 'linear-gradient(90deg, rgba(0,240,255,0.15), rgba(244,114,182,0.15))', border: '1px solid rgba(0,240,255,0.3)', fontSize: 11, fontWeight: 700, color: '#e0f7ff' }}>
            ...
            <section>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <button
                  ...
                  style={{
                    background: 'none', border: '1px solid rgba(167,139,250,0.35)', borderRadius: 5,
                    color: '#a78bfa', fontSize: 11, cursor: 'pointer', padding: '4px 12px',
                    transition: 'all 0.15s',
                  }}
                >
                  + New Playlist
      ...
      {newPlModal && (
        ...
        >
          <div
            style={{
              background: '#0e0e1a', border: '1px solid rgba(167,139,250,0.25)',
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: newPlName.trim() ? 'rgba(124,58,237,0.2)' : 'rgba(255,255,255,0.05)',
                    border: '1px solid rgba(167,139,250,0.35)',
                    color: newPlName.trim() ? '#c4b5fd' : '#444',
